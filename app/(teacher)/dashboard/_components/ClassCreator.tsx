        ...
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-6">
            <div className="lg:col-span-6 bg-slate-800/90 border border-slate-700 p-4 rounded-3xl shadow-xl sm:p-6">
                ...
                <form onSubmit={onCreateClass} className="space-y-4">
                    ...
                    <button type="submit" disabled={isCreatingClass || !className.trim() || !subject.trim()} aria-busy={isCreatingClass} className="w-full py-3 px-4 bg-gradient-to-r from-blue-600 to-indigo-600 hover:from-blue-500 hover:to-indigo-500 disabled:cursor-not-allowed disabled:opacity-60 text-white font-semibold rounded-xl shadow-lg transition-all flex justify-center items-center gap-2 cursor-pointer mt-6">{isCreatingClass ? <LoaderCircle className="w-5 h-5 animate-spin" /> : <Plus className="w-5 h-5" />} {isCreatingClass ? 'Membuat Kelas...' : 'Simpan & Buat Kelas'}</button>
                </form>
            </div>
