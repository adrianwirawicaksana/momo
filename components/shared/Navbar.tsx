...
const Navbar = () => {
    ...
    // Pastikan komponen sudah ter-mount di Client sebelum membaca pathname secara dinamis
    useEffect(() => {
        setIsMounted(true);
    }, []);
    ...
    return (
        <nav className="sticky top-0 w-full h-20 bg-gray-800 border-b-2 border-gray-900 z-50">
            <div className="h-full w-full max-w-5xl lg:max-w-6xl mx-auto flex items-center justify-between px-4 sm:px-6 lg:px-8">
                <Link href="/" className="flex items-center gap-2.5 hover:opacity-90 transition-opacity">
                    ...
                    />
                    <span className="text-3xl sm:text-4xl font-extrabold text-blue-400 font-[family-name:var(--font-fredoka)] tracking-wide">
                        MoMo
                    </span>
                </Link>
                ...
                <div className="flex items-center gap-4">
                    <ShiningButton className="py-2 px-3 sm:px-4 font-extrabold uppercase" href={buttonHref} variant="blue">
                        <span className="flex items-center justify-center gap-1.5 sm:gap-2">
                            ...
                            {/* Responsive Text: Tampil "MULAI" di HP (< sm) & "MULAI BELAJAR" di Layar Desktop (>= sm) */}
                            {isGuidePage ? (
                                <span>
